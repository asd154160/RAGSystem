"""企业RAG问答API — Phase 7: LangGraph 编排 + 会话保存"""
import uuid
import json
import logging
...
from app.services import llm_service, audit_service, metrics_service
...
async def _save_session(
    ...
    if not session:
        ...
        session.updated_at = None  # Trigger onupdate
...
async def chat(
    ...
    if not kb_ids:
        return ChatResponse(answer="没有可用的企业知识库。", sources=[], session_id=session_id)
    ...
    enable_rewrite = any(cfg.enable_query_rewrite for cfg in kb_configs.values()) if kb_configs else True
    ...
    # Full pipeline
    metrics_service.increment_counter("rag_query")
    t0 = time.time()
    ...
    messages = [
        {"role": "system", "content": "你是一个企业知识库问答助手。请根据提供的文档片段回答用户问题。"},
    ...
    metrics_service.record_timing("rag_chat_total", (time.time() - t0) * 1000)
    if low_confidence:
        metrics_service.increment_counter("low_confidence")
